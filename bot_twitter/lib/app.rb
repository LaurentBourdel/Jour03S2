...
def twit_journalists(client)
	...
	#twit_to_list = list_journalists.sample(5)
	#twit_cc = "@the_hacking_pro"
	#twit_h_tag = "#bonjour_monde"
	#twit_to_list.each_with_index do |value, index|
	#twit_text = "Bonjour #{value}, tu fais un super travail! Merci... #{twit_h_tag}, #{twit_cc}"
	#puts "> #{twit_text}"
	#client.update(twit_text)
	#end
end

def get_client_twit(client)
	#tweets = client.user_timeline('jcunniet', count: 1)
	#tweets.each { |tweet| puts tweet.full_text }
	#client.search("#bonjour_monde", result_type: "recent").take(20) do |tweet|
	#client.favorite(tweet.user)

	#client.search("#bonjour_monde", result_type: "recent").take(20).each do |like|

	tweets = client.search("#bonjour_monde", result_type: "recent").take(20)
	client.favorites(tweets)
...
twit_journalists(client)
get_client_twit(client)
